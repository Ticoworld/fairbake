...
type Filter = "ALL" | "LIVE" | "UPCOMING" | "CLOSED";
...
export function ExplorePage() {
  ...
  const [filter, setFilter] = useState<Filter>(() => params.get("status") === "CLOSED" ? "CLOSED" : "ALL");
  ...
  const sales = useMemo(() => (query.data ?? []).filter((sale) => {
    ...
    const matchesFilter = filter === "ALL" || (filter === "CLOSED" ? status === "SUCCESS" || status === "FAILED" || status === "ENDED_AWAITING_FINALIZATION" : status === filter);
  ...
  return <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8 sm:py-10"><div className="flex items-center justify-between gap-5 border-b border-line pb-5"><h1 className="text-2xl font-semibold tracking-[-0.04em]">Explore</h1><Link href="/create" className="button-secondary rounded-lg">Create sale</Link></div><div className="mt-6 flex flex-col gap-4 border-b border-line pb-4 xl:flex-row xl:items-center xl:justify-between"><div className="flex flex-wrap gap-1">{(["ALL", "LIVE", "UPCOMING", "CLOSED"] as Filter[]).map((value) => <button key={value} className={`border-b-2 px-3 py-2 text-[11px] font-bold uppercase tracking-[0.12em] transition ${filter === value ? "border-ink text-ink" : "border-transparent text-moss hover:border-line hover:text-ink"}`} onClick={() => setFilter(value)}>{value}</button>)}</div><div className="flex flex-col gap-3 sm:flex-row"><label className="relative block w-full sm:w-80"><Search size={16} className="absolute left-3 top-3 text-moss"/><input className="field h-10 pl-9" placeholder="Search name, symbol, or address..." value={search} onChange={(event) => setSearch(event.target.value)}/></label><label className="flex h-10 items-center gap-2 border-b border-line px-2 text-xs text-moss"><span>Sort</span><select className="bg-transparent py-2 font-semibold text-ink outline-none" value={sort} onChange={(event) => setSort(event.target.value as Sort)}><option value="ENDING">Ending soon</option><option value="NEWEST">Newest</option><option value="COMMITTED">Most contributed</option><option value="PARTICIPANTS">Most participants</option></select></label></div></div><div className="mt-6">{query.isLoading ? <LaunchTableSkeleton rows={6}/> : query.error ? <ChainState isLoading={false} error={query.error} onRetry={() => void query.refetch()}/> : <><div className="mb-5 flex items-center justify-between gap-3 text-sm text-moss"><span>{sales.length} {sales.length === 1 ? "sale" : "sales"}</span></div><LaunchGrid sales={sales} empty="No sales match these filters."/></>}</div></main>;
}
