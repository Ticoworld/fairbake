    ...
    <div className="lg:pl-[88px]">
      <header className="relative sticky top-0 z-40 border-b border-line bg-paper/95 backdrop-blur">
        <div className="mx-auto flex h-[72px] max-w-[1480px] items-center justify-between gap-5 px-5 sm:px-8 lg:px-10">
          ...
          <label className="relative hidden min-w-0 flex-1 md:block md:max-w-[420px]" aria-label="Search launches"><Search size={15} className="absolute left-3 top-3 text-moss"/><input className="field h-10 bg-white/50 pl-9" placeholder="Search launches..." value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={submitSearch}/></label>
          <div className="flex items-center gap-3"><a href="https://cookiescan.io" target="_blank" rel="noreferrer" className="hidden items-center gap-1 text-xs font-semibold text-moss hover:text-ink sm:inline-flex">CookieScan <ArrowUpRight size={13}/></a><Link href="/create" onClick={() => startNavigation("/create")} className="hidden items-center gap-2 rounded-lg bg-ink px-3.5 py-2.5 text-xs font-semibold text-paper transition hover:bg-orange sm:inline-flex"><Plus size={14}/> Create</Link><WalletButton/><button className="grid h-10 w-10 place-items-center rounded-lg border border-line lg:hidden" onClick={() => setOpen((value) => !value)} aria-label="Toggle menu">{open ? <X size={18}/> : <Menu size={18}/>}</button></div>
        </div>
        {open && <div className="border-t border-line px-5 py-5 lg:hidden"><nav className="grid gap-2">{links.map(({ label, href, icon: Icon }) => { const active = href === "/" ? pathname === "/" : pathname.startsWith(href); return <Link className={`flex items-center gap-3 rounded-lg px-3 py-3 text-sm font-semibold ${active ? "bg-cream" : "hover:bg-cream"}`} href={href} key={href} onClick={() => startNavigation(href)} aria-current={active ? "page" : undefined}><Icon size={17}/>{label}</Link>; })}<Link className="flex items-center gap-3 rounded-lg bg-ink px-3 py-3 text-sm font-semibold text-paper" href="/create" onClick={() => startNavigation("/create")}><Plus size={17}/>Create launch</Link></nav></div>}
        {navigating && <div className="route-progress" aria-label="Loading next page"/>}
      </header>
      {children}
      <footer className="border-t border-line"><div className="mx-auto flex max-w-[1480px] flex-col gap-3 px-5 py-8 text-xs text-moss sm:flex-row sm:items-center sm:justify-between sm:px-8 lg:px-10"><p>Fair launches, settled by the chain.</p><div className="flex gap-5"><Link href="/explore" onClick={() => startNavigation("/explore")}>Explore</Link><Link href="/create" onClick={() => startNavigation("/create")}>Create</Link><a href="https://cookiescan.io" target="_blank" rel="noreferrer">CookieScan <ArrowUpRight size={11} className="inline"/></a></div></div></footer>
    </div>
  </div>;
}
