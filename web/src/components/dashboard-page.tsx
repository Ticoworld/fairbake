...
export function DashboardPage() {
  ...
  if (!publicKey) return <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8 sm:py-10"><div className="flex items-center justify-between gap-5 border-b border-line pb-5"><h1 className="text-2xl font-semibold tracking-[-0.04em]">Dashboard</h1><Link className="button-secondary" href="/explore">Explore sales <ArrowUpRight size={15}/></Link></div><section className="mt-16 max-w-2xl border-y border-line py-9"><div className="flex items-center gap-3 text-moss"><WalletCards size={18}/><span className="font-mono text-[10px] uppercase tracking-[0.16em]">Wallet required</span></div><h2 className="mt-6 text-3xl font-semibold tracking-[-0.05em]">Connect wallet.</h2><p className="mt-3 max-w-lg text-sm leading-6 text-moss">Your contributions and created sales will appear here.</p><div className="mt-7 flex flex-wrap items-center gap-4"><WalletButton/><Link className="text-sm font-semibold text-moss underline underline-offset-4 hover:text-ink" href="/explore">Browse sales</Link></div></section></main>;

  const loading = positionsQuery.isLoading || createdQuery.isLoading;
  return <main className="mx-auto max-w-7xl px-5 py-14 sm:px-8 sm:py-20"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><h1 className="text-5xl font-semibold tracking-[-0.06em]">Your activity.</h1><p className="mt-5 font-mono text-xs text-moss">{shorten(publicKey.toBase58(), 10, 8)}</p></div><Link className="button-secondary" href="/explore">Explore sales <ArrowUpRight size={15}/></Link></div>{loading && <DashboardSkeleton/>}{!loading && <div className="mt-12 grid gap-14"><section><div className="mb-5 flex items-baseline gap-3"><h2 className="text-xl font-semibold">Your contributions</h2><span className="font-mono text-xs text-moss">{positionsQuery.data?.length ?? 0}</span></div>{positionsQuery.error ? <p className="border border-orange/30 bg-orange/5 p-4 text-sm text-orange">Could not load your contributions from Cookie.</p> : positionsQuery.data?.length ? <div className="grid gap-3">{positionsQuery.data.map((position, index) => <PositionRow key={position.address.toBase58()} position={position} sale={saleQueries[index]?.data}/>)}</div> : <Empty title="No contributions yet." body="Your contribution will appear here after you join a sale." href="/explore" label="Explore live sales"/>}</section><section><div className="mb-5 flex items-baseline gap-3"><h2 className="text-xl font-semibold">Your sales</h2><span className="font-mono text-xs text-moss">{createdQuery.data?.length ?? 0}</span></div>{createdQuery.error ? <p className="border border-orange/30 bg-orange/5 p-4 text-sm text-orange">Could not load your sales from Cookie.</p> : createdQuery.data?.length ? <div className="grid gap-3">{createdQuery.data.map((sale) => <CreatedRow key={sale.address.toBase58()} sale={sale}/>)}</div> : <Empty title="No sales created yet." body="Create a fixed-window sale and record its parameters on-chain." href="/create" label="Create sale"/>}</section></div>}</main>;
}
...
function PositionRow({ position, sale }: { position: any; sale: any }) {
  ...
  const claimStatus = position.data.claimed ? "Claimed" : (status === "SUCCESS" || status === "FAILED" ? "Claim available" : "Awaiting settlement");
  return <div className="border border-line bg-paper p-5 transition hover:border-ink/30"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-3"><h3 className="font-semibold">{title}</h3>{status && <StatusPill status={status}/>}</div><p className="mt-2 text-sm text-moss">Contribution {formatUnits(position.data.contributed, 9)} COOK · {claimStatus}</p></div><div className="flex flex-wrap items-center gap-4 sm:justify-end"><PositionAction position={position} sale={sale}/><Link href={`/launch/${position.data.sale.toBase58()}`} className="inline-flex items-center gap-1 text-xs font-semibold text-moss underline underline-offset-4 hover:text-ink">View sale <ArrowUpRight size={13}/></Link></div></div></div>;
}
...
function CreatedRow({ sale }: { sale: any }) {
  ...
  return <div className="border border-line bg-paper p-5 transition hover:border-ink/30"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-3"><h3 className="font-semibold">{title}</h3><StatusPill status={status}/></div><p className="mt-2 text-sm text-moss">{formatUnits(sale.data.totalCommitted, 9)} / {formatUnits(sale.data.hardCap, 9)} COOK total contributed · {sale.data.buyerCount.toString()} participants</p></div><div className="flex flex-wrap items-center gap-4 sm:justify-end"><CreatorActions sale={sale}/><Link href={`/launch/${sale.address.toBase58()}`} className="inline-flex items-center gap-1 text-xs font-semibold text-moss underline underline-offset-4 hover:text-ink">View sale <ArrowUpRight size={13}/></Link></div></div></div>;
}
...
function PositionAction({ position, sale }: { position: any; sale: any }) {
  ...
  return <div className="flex items-center gap-2">{error && <span className="text-xs text-orange">{error}</span>}<button className="button-primary px-3 py-2 text-xs" disabled={!signer || busy} onClick={run}>{busy ? "Claiming…" : refund ? "Claim full COOK refund" : "Claim tokens & any refund"}</button></div>;
}
...
function CreatorActions({ sale }: { sale: any }) {
  ...
  return <div className="flex flex-wrap items-center gap-2">{error && <span className="text-xs text-orange">{error}</span>}{status === "ENDED_AWAITING_FINALIZATION" && <button className="button-primary px-3 py-2 text-xs" disabled={!signer || Boolean(busy)} onClick={(event) => void run(event, "finalize", () => finalizeSale(signer!, sale.address))}>{busy === "finalize" ? "Closing sale…" : "Close and settle sale"}</button>}{status === "SUCCESS" && !sale.data.proceedsWithdrawn && <button className="button-primary px-3 py-2 text-xs" disabled={!signer || Boolean(busy)} onClick={(event) => void run(event, "proceeds", () => withdrawProceeds(signer!, sale.address))}>{busy === "proceeds" ? "Withdrawing…" : "Withdraw sale proceeds"}</button>}{inventoryReady && !sale.data.inventoryWithdrawn && <button className="button-secondary px-3 py-2 text-xs" disabled={!signer || Boolean(busy)} onClick={(event) => void run(event, "inventory", () => withdrawInventory(signer!, sale.address, sale.data.mint, sale.data.creatorTokenAccount))}>{busy === "inventory" ? "Withdrawing…" : "Withdraw remaining sale tokens"}</button>}</div>;
}
